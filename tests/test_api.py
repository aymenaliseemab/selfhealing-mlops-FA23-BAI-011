...
def test_predict_returns_label_and_confidence():
    ...
    Uses the assigned APP test sentence (row FA23-BAI-011)."""
    r = requests.post(f"{BASE_URL}/predict", json={"text": "The cinematography was breathtaking and the performances were outstanding"})
    assert r.status_code == 200
    data = r.json()
